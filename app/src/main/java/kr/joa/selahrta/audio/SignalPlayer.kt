 ...
 *
 * **[start] 와 [stop] 은 한 스레드에서만 부른다**(앱에서는 주 스레드다).
 * 상한이 지켜지는 근거가 「`start` 가 먼저 `stop` 을 불러 같은 수명주기
 * 자물쇠를 반드시 거친다」이기 때문이다 — 여러 곳에서 동시에 [start] 를
 * 불러도 안전하다는 보장은 **검증하지 않았다**(독립 검증자 지적).
 * 내보내는 스레드가 스스로 끝나는 경로는 그와 별개로 안전하다.
 */
class SignalPlayer(
    /**
     ...
     *
     * **[stop] 과 같은 한 스레드에서만 부른다**(클래스 설명 참고).
     *
