...
class BandAnalyzer(
     ...
     * 빠진다(독립 재검증 F05).
     *
     * 왜 경계를 빼는가. 경계 근처 손실은 **주파수와 무관하게** 모든 밴드에
     * 생긴다 — 유한한 창을 쓰는 한 주엽이 경계를 넘기 때문이다. 그것까지
     * 넣어 판정하면 31개 밴드가 전부 「못 믿음」이 되어 화면이 아무것도
     * 구별해 주지 못한다. 그래서 「이 밴드가 제 대역을 담는가」를 가리는
     * 데는 가운데 구간을 쓰고, **그 사실을 화면에 적는다.**
     */
