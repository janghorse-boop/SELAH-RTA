...
# Expected exit code on 6357669: 0. Assertions confirm observed defects, not desired behavior.
$ErrorActionPreference = 'Stop'
$scratch = Join-Path ([IO.Path]::GetTempPath()) ('selah-independent-' + [guid]::NewGuid().ToString('N'))
New-Item -ItemType Directory -Path $scratch | Out-Null
$archive = Join-Path $scratch '6357669.zip'
...
& git -c "safe.directory=$repoPath" -C $Repository archive 6357669 -o $archive
if ($LASTEXITCODE -ne 0) { throw 'Could not archive review target 6357669' }
...
$files += Join-Path $PSScriptRoot 'Phase9Probe.kt'
...
& $Java -cp "$classes;$cp" Phase9ProbeKt
exit $LASTEXITCODE
