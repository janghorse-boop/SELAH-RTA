...
# Exit 0 means the slow-release capacity bypass was reproduced, not fixed.
$ErrorActionPreference = 'Stop'
$scratch = Join-Path ([IO.Path]::GetTempPath()) ('selah-independent-' + [guid]::NewGuid().ToString('N'))
New-Item -ItemType Directory -Path $scratch | Out-Null
$archive = Join-Path $scratch '365611e.zip'
...
& git -c "safe.directory=$repoPath" -C $Repository archive 365611e -o $archive
if ($LASTEXITCODE -ne 0) { throw 'Could not archive review target 365611e' }
...
$files += Join-Path $PSScriptRoot 'SpFollowupProbe.kt'
...
& $Java -cp "$classes;$cp" kr.joa.selahrta.audio.SpFollowupProbeKt
exit $LASTEXITCODE
