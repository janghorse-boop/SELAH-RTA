...
# Expected exit code on de4f0cc: 0. Assertions confirm observed defects, not desired behavior.
$ErrorActionPreference = 'Stop'
$scratch = Join-Path ([IO.Path]::GetTempPath()) ('selah-independent-' + [guid]::NewGuid().ToString('N'))
New-Item -ItemType Directory -Path $scratch | Out-Null
$archive = Join-Path $scratch 'de4f0cc.zip'
...
& git -c "safe.directory=$repoPath" -C $Repository archive de4f0cc -o $archive
if ($LASTEXITCODE -ne 0) { throw 'Could not archive review target de4f0cc' }
...
$files += Join-Path $PSScriptRoot 'GateProbe.kt'
...
& $Java -cp "$classes;$cp" kr.joa.selahrta.ui.GateProbeKt
exit $LASTEXITCODE
