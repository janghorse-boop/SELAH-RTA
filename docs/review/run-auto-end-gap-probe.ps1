...
# Inspect reproduced=true and pending > cap for the race; exit 0 alone is not evidence of reproduction or a fix.
$ErrorActionPreference = 'Stop'
$scratch = Join-Path ([IO.Path]::GetTempPath()) ('selah-independent-' + [guid]::NewGuid().ToString('N'))
New-Item -ItemType Directory -Path $scratch | Out-Null
$archive = Join-Path $scratch 'f026cf1.zip'
...
& git -c "safe.directory=$repoPath" -C $Repository archive f026cf1 -o $archive
if ($LASTEXITCODE -ne 0) { throw 'Could not archive review target f026cf1' }
...
$files += Join-Path $PSScriptRoot 'AutoEndGapProbe.kt'
...
& $Java -cp "$classes;$cp" kr.joa.selahrta.audio.AutoEndGapProbeKt
exit $LASTEXITCODE
