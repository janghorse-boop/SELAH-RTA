...
# Exit 0 confirms locking/manual-stop handling and reproduces autonomous-error release accounting gap.
$ErrorActionPreference = 'Stop'
$scratch = Join-Path ([IO.Path]::GetTempPath()) ('selah-independent-' + [guid]::NewGuid().ToString('N'))
New-Item -ItemType Directory -Path $scratch | Out-Null
$archive = Join-Path $scratch '430aca8.zip'
...
& git -c "safe.directory=$repoPath" -C $Repository archive 430aca8 -o $archive
if ($LASTEXITCODE -ne 0) { throw 'Could not archive review target 430aca8' }
...
$files += Join-Path $PSScriptRoot 'RcReviewProbe.kt'
...
& $Java -cp "$classes;$cp" kr.joa.selahrta.audio.RcReviewProbeKt
exit $LASTEXITCODE
