...
# Exit 0 verifies lifecycle-lock blocking and cap preservation during a controlled pending-registration delay.
$ErrorActionPreference = 'Stop'
$scratch = Join-Path ([IO.Path]::GetTempPath()) ('selah-independent-' + [guid]::NewGuid().ToString('N'))
New-Item -ItemType Directory -Path $scratch | Out-Null
$archive = Join-Path $scratch '0204f7f.zip'
...
& git -c "safe.directory=$repoPath" -C $Repository archive 0204f7f -o $archive
if ($LASTEXITCODE -ne 0) { throw 'Could not archive review target 0204f7f' }
...
$files += Join-Path $PSScriptRoot 'DetachHandoffProbe.kt'
...
# Full suite runs separately.
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
Write-Output "Independent verification scratch directory: $scratch"
& $Java -cp "$classes;$cp" kr.joa.selahrta.audio.DetachHandoffProbeKt
exit $LASTEXITCODE
