...
# Expected exit code on bb30a09: 0. Assertions check preservation and rejection of late snapshots.
$ErrorActionPreference = 'Stop'
$scratch = Join-Path ([IO.Path]::GetTempPath()) ('selah-independent-' + [guid]::NewGuid().ToString('N'))
New-Item -ItemType Directory -Path $scratch | Out-Null
$archive = Join-Path $scratch 'bb30a09.zip'
...
& git -c "safe.directory=$repoPath" -C $Repository archive bb30a09 -o $archive
if ($LASTEXITCODE -ne 0) { throw 'Could not archive review target bb30a09' }
...
$cp = $jars -join ';'
$probe = Join-Path $PSScriptRoot 'CopyOnWriteRemoveAllProbe.kt'
& $Java -cp $cp org.jetbrains.kotlin.cli.jvm.K2JVMCompiler -no-stdlib -no-reflect -jvm-target 17 -classpath $cp -d $classes $probe
if ($LASTEXITCODE -ne 0) { throw 'Probe compilation failed' }
& $Java -cp "$classes;$cp" CopyOnWriteRemoveAllProbeKt
exit $LASTEXITCODE
