...
# DSP and pure CaptureGeneration tests. No production or global settings are changed.
# Expected exit code on b220030: 0. Assertions confirm observed defects, not desired behavior.
$ErrorActionPreference = 'Stop'
$scratch = Join-Path ([IO.Path]::GetTempPath()) ('selah-independent-' + [guid]::NewGuid().ToString('N'))
New-Item -ItemType Directory -Path $scratch | Out-Null
$archive = Join-Path $scratch 'b220030.zip'
...
& git -c "safe.directory=$repoPath" -C $Repository archive b220030 -o $archive
if ($LASTEXITCODE -ne 0) { throw 'Could not archive review target b220030' }
...
$files += Join-Path $snapshot 'app\src\main\java\kr\joa\selahrta\audio\CaptureGeneration.kt'
$files += Join-Path $snapshot 'app\src\test\java\kr\joa\selahrta\audio\CaptureGenerationTest.kt'
$files += Join-Path $PSScriptRoot 'CombinedProbe.kt'
...
$tests += 'kr.joa.selahrta.audio.CaptureGenerationTest'
...
& $Java -cp "$classes;$cp" CombinedProbeKt
exit $LASTEXITCODE
