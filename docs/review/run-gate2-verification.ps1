...
# Expected exit code on b8b4fa3: 0. Assertions check preservation and rejection of late snapshots.
$ErrorActionPreference = 'Stop'
$scratch = Join-Path ([IO.Path]::GetTempPath()) ('selah-independent-' + [guid]::NewGuid().ToString('N'))
New-Item -ItemType Directory -Path $scratch | Out-Null
$archive = Join-Path $scratch 'b8b4fa3.zip'
...
& git -c "safe.directory=$repoPath" -C $Repository archive b8b4fa3 -o $archive
if ($LASTEXITCODE -ne 0) { throw 'Could not archive review target b8b4fa3' }
...
$files += Join-Path $PSScriptRoot 'Gate2Probe.kt'
...
& $Java -cp "$classes;$cp" kr.joa.selahrta.ui.Gate2ProbeKt
exit $LASTEXITCODE
